...
<head>
  ...
  <style>
    ...
    footer::before {
      ...
      position: absolute;
      inset: 0;
      background: linear-gradient(120deg, rgba(255, 255, 255, 0.05), rgba(212, 175, 55, 0.05));
...
<body class="text-[#e5e5e5] font-[IBM_Plex_Sans_Thai] bg-gradient-to-b from-neutral-950 via-black to-neutral-900 text-gray-100 flex flex-col min-h-screen">
  ...
  <nav class="glass-dark fixed w-full top-0 z-50 shadow-[0_0_25px_rgba(212,175,55,0.15)] border-b border-neutral-800">
    <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
      {{-- โลโก้ --}}
      <a href="{{ route('home') }}" class="flex items-center space-x-2 text-yellow-400 font-[Playfair_Display] text-2xl tracking-wide">
    ...
    <div id="mobileMenu" class="hidden md:hidden bg-neutral-900/95 border-t border-neutral-800">
      <div class="flex flex-col text-center py-4 space-y-1 font-medium">
        <a href="{{ route('home') }}" class="hover:bg-neutral-800 py-3 text-gray-300 hover:text-yellow-400">หน้าแรก</a>
        <a href="{{ route('about') }}" class="hover:bg-neutral-800 py-3 text-gray-300 hover:text-yellow-400">เกี่ยวกับเรา</a>
        <a href="{{ route('contact') }}" class="hover:bg-neutral-800 py-3 text-gray-300 hover:text-yellow-400">ติดต่อเรา</a>
      </div>
    </div>
  </nav>
  ...
  {{-- 🖼️ Hero --}}
  @if (Request::is('/'))
  <section class="relative h-[75vh] flex items-center justify-center text-center text-yellow-50 overflow-hidden mt-[67px]">
    <img src="images\1\1_1.jpg"
    ...
    <div class="relative z-10 px-6 fade-in">
      ...
      <a href="#tours" class="btn-gold inline-flex items-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke-width="2"
          stroke="currentColor">
